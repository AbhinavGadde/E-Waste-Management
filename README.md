...
## Notes
- Set `GEMINI_API_KEY` in the backend environment to enable Google Gemini image inspection. Optionally, override the model with `GEMINI_MODEL_NAME` (defaults to `gemini-1.5-flash-001`). The backend will automatically fall back to other 1.5 Flash variants if the chosen model is unavailable.
